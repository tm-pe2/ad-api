...
const updateInvoiceStatus = `
    ...
    RETURNING invoice_id
`;
const selectOverdueNotReminded = `SELECT
i.id,
i.contract_id,
i.supplier_id,
i.price,
...
LEFT JOIN ${TABLES.ROLES} as r ON ur.role_id = r.id
LEFT JOIN ${TABLES.INVOICES_STATUSES} as ins ON i.id = ins.invoice_id
LEFT JOIN ${TABLES.TARIFFS} as t ON t.id = i.tariff_id WHERE (ins.status_id = $1 OR ins.status_id = $2) AND i.due_date <= $3`;
...
export const invoiceQueries = {
    ...
    getInvoiceById: getAllInvoices + `
        WHERE i.id = $1
    ` + groupBy,
    getLateNotReminded: selectOverdueNotReminded + groupBy
